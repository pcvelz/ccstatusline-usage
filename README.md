...
- [pcvelz/ccstatusline-usage](https://github.com/pcvelz/ccstatusline-usage): **Session-state contract** - the llama-swap widgets read `GET /api/sessions` (`llama-swap.sessions/v1`), so phase, context and rate are computed once server-side. Slot Status shows priority (`P10`), Slot Throughput the rate kind.
- [pcvelz/ccstatusline-usage](https://github.com/pcvelz/ccstatusline-usage): **Cache-aware fallback** - without `/api/sessions` the widgets fall back to `/api/events` + `/api/slots`, now counting the reused KV-cache prefix (a resumed retry no longer reads `6k/102k`).
- [pcvelz/ccstatusline-usage](https://github.com/pcvelz/ccstatusline-usage): **Uniform bar widths** - Fable, Opus, Sonnet, Extra Usage and the timer bars follow the same 4/8/16 narrow/medium/full widths as Session and Weekly.
- [pcvelz/ccstatusline-usage](https://github.com/pcvelz/ccstatusline-usage): **No more cut-off lines** - in tmux the width comes from the session's own pane, and a line that doesn't fit wraps at widget boundaries instead of ending in `...`.
...
- [sirmalloc/ccstatusline](https://github.com/sirmalloc/ccstatusline): **🩺 Claude service health** - Added a `Claude Status` widget with live severity, a cached 48-hour incident-history strip, stale-data fallback, and graceful `?` output when status data is unavailable.
- [sirmalloc/ccstatusline](https://github.com/sirmalloc/ccstatusline): **🙈 Unified conditional hiding** - Numeric, Git, JJ, usage, cache, and other widgets now share an `h` checklist for supported hide conditions, with automatic migration of existing settings and optional merge-target hiding for decorative text or symbols.
- [sirmalloc/ccstatusline](https://github.com/sirmalloc/ccstatusline): **🔢 Configurable number formatting** - Numeric widgets can use precise, compact, or whole-number styles per widget or globally by token, speed, percent, memory, and cost type, while advanced configs can set decimal precision explicitly.
...
- [sirmalloc/ccstatusline](https://github.com/sirmalloc/ccstatusline): **⚠️ Git conflict display controls** - Git Conflicts can hide when the count is zero or show either `⚠0` or a customizable clean glyph when the tree is conflict-free.
- [sirmalloc/ccstatusline](https://github.com/sirmalloc/ccstatusline): **🩹 Self-healing usage locks** - Usage widgets ignore impossible fetch-lock deadlines more than 24 hours ahead, allowing poisoned locks caused by clock jumps or old test artifacts to recover on the next render.
- [sirmalloc/ccstatusline](https://github.com/sirmalloc/ccstatusline): **🧹 Bounded Git cache cleanup** - Failed persistent Git-cache writes clean up their temporary file and reuse one stable fallback name, preventing Windows file locks from leaking thousands of orphaned temp files.
- [sirmalloc/ccstatusline](https://github.com/sirmalloc/ccstatusline): **📊 Consistent timer bars** - Block Timer, Block Reset Timer, and Weekly Reset Timer now round progress-bar fill to the nearest cell, matching the other progress widgets.
...
> 🪟 **Windows Support:** PowerShell examples, installation notes, fonts, troubleshooting, WSL, and Windows Terminal configuration are in the [Windows Support](#-windows-support) section below.
...
| command | median |
| --- | --- |
| `bunx -y ccstatusline-usage@latest` | 633 ms |
| `bunx -y ccstatusline-usage@2.4.14` | 202 ms |
| `bunx -y ccstatusline-usage` | 207 ms |

Dropping `@latest` is worth about 430 ms per repaint there. npm does not behave this way: `npx -y
ccstatusline-usage@latest` and `npx -y ccstatusline-usage@2.4.14` measured 1082 ms and 1135 ms, so pinning buys
nothing under `npx`. A **Pinned global install**, which writes `"command": "ccstatusline"`, avoids the
resolution entirely on both.

For pinned installs, launch the TUI with `npx -y ccstatusline-usage@latest` or `bunx -y ccstatusline-usage@latest`, then choose **Pinned global install**. The TUI pins the active version by installing it globally and writing `"command": "ccstatusline"` to `settings.json`; afterward, you can run `ccstatusline` directly to open the TUI.
