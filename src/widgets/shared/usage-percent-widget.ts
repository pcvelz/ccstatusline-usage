...
type UsagePercentField = 'sessionUsage' | 'weeklyUsage' | 'weeklySonnetUsage' | 'weeklyOpusUsage' | 'weeklyFableUsage';

interface UsageCursorOptions { cursorPercent: number }

interface UsagePercentWidgetKindConfig {
    label: string;
    // Fork: label used on narrow terminals (see getUsageLabel).
    shortLabel?: string;
    displayName: string;
    description: string;
...
function resolveUsageWindow(kind: UsagePercentWidgetKind, data: RenderUsageData, context: RenderContext): UsageWindowMetrics | null {
    ...
    return resolveWeeklyFableUsageWindow(data);
}
...
export function renderUsagePercentWidgetValue(
    ...
    const label = config.shortLabel ? getUsageLabel(context, config.label, config.shortLabel) : config.label;
    ...
    if (context.isPreview) {
        ...
        return renderUsageDisplay(item, label, renderedPercent, format, context, () => showCursor ? { cursorPercent: 50 } : undefined);
    }
    ...
    return renderUsageDisplay(item, label, renderedPercent, format, context, () => {
        if (!showCursor) {
            return undefined;
        }
