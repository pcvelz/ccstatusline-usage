...
export class ContextBarWidget implements Widget {
    ...
    render(item: WidgetItem, context: RenderContext, settings: Settings): string | null {
        ...
        if (isBarSliderMode(displayMode)) {
            ...
            const sliderDisplay = displayMode === 'slider' ? `${slider} ${usedK}k/${totalStr} (${Math.round(clampedPercent)}%)` : slider;
            return item.rawValue ? sliderDisplay : `Context: ${sliderDisplay}`;
        }

        const barWidth = displayMode === 'progress' ? 32 : 16;
        const display = `${makeUsageProgressBar(clampedPercent, barWidth)} ${usedK}k/${totalStr} (${Math.round(clampedPercent)}%)`;
