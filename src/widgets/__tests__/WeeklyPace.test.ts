...
describe('WeeklyPaceWidget', () => {
    ...
    it('renders a real zero with no reset of its own (#534: model-scoped 0% is real usage)', () => {
        // Placeholder filtering happens in usage-fetch; a defined 0 reaching
        // the widget is a model left unused this window, so the verdict holds.
        mockResolveWeeklyPaceWindow.mockReturnValue(makeWindow(50));
        const item: WidgetItem = { ...BASE_ITEM, metadata: { source: 'opus' } };
        expect(render({ usageData: { weeklyUsage: 50, weeklyOpusUsage: 0 } }, item)).toBe('Opus D4/7: Underusing -50%');
    });
