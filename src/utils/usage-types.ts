...
// Placeholder filtering happens once, in usage-fetch (isPlaceholderUsageApiLimit):
// a defined field is a real reading. Since upstream #534 a model-scoped 0% with
// no resets_at is real zero usage (the model is simply unused this window), so
// a per-model pace built on it is truthful and must not be hidden here.
export function hasTrustworthyPaceUsage(data: UsageData, source: WeeklyPaceSource): boolean {
    return getUsageNumberField(data, source.usageField) !== undefined;
}
