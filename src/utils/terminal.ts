...
function probeTerminalWidth(): number | null {
    ...
    // Equivalent to: tmux display-message -p -t "$TMUX_PANE" '#{pane_width}'
    // (CCSTATUSLINE_WIDTH is honored earlier, in getTerminalWidth.)
    if (process.env.TMUX) {
        const pane = process.env.TMUX_PANE;
        const targetArgs = pane && /^%\d+$/.test(pane) ? ['-t', pane] : [];
        try {
            const output = execFileSync(
                'tmux',
                ['display-message', '-p', ...targetArgs, '#{pane_width}'],
                { encoding: 'utf8', stdio: ['pipe', 'pipe', 'ignore'], timeout: 2000, windowsHide: true }
            ).trim();
